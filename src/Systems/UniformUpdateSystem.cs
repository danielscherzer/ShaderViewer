...
internal sealed partial class UniformUpdateSystem : AEntitySetSystem<float>
{
	public UniformUpdateSystem(World world) : base(world, CreateEntityContainer, false)
	{
		...
		void ActiveUpdaters(Components.Uniforms uniforms)
		{
		...
		world.SubscribeEntityComponentAddedOrChanged((in Entity _, in Components.Uniforms component) => ActiveUpdaters(component));
	}

	private readonly List<IUniformUpdater> activeUpdaters = new();

	[Update]
	private void Update(float deltaTime, in Components.Uniforms uniforms)
	{
