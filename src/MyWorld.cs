...
internal class MyWorld
{
	internal static World Create(GameWindow window)
	{
		...
		world.SubscribeWorldComponentAddedOrChanged((World world, in ShaderFile shaderFile) =>
		{
			...
			if(ReadShaderSourceSystem.Load(world, shaderFile.Name))
			{
		...
		world.SubscribeWorldComponentAddedOrChanged((World _, in AlwaysOnTop alwaysOnTop) =>
		{
			unsafe
			{
				GLFW.SetWindowAttrib(window.WindowPtr, WindowAttribute.Floating, alwaysOnTop);
			}
		});
